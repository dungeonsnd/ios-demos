...
#import "VoiceConv/VoiceConverter.h"
@import AVFoundation;
@import AudioToolbox;
...
- (IBAction)playAndroidAMR:(UIButton *)sender;
...
#warning amr转wav
            ...
            NSLog(@"#### convertedPath=%@", convertedPath);
...
- (IBAction)playAndroidAMR:(UIButton *)sender {
    ...
    NSString *amrPath=[[NSBundle mainBundle]pathForResource:@"38696863-8903-4303-91cc-21dfaaffa902.amr" ofType:nil];
    
    NSString *containerPath=[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSString *wavSavePath=[containerPath stringByAppendingPathComponent:@"38696863-8903-4303-91cc-21dfaaffa902.wav"];
    
    NSLog(@"**** amrPath=%@", amrPath);
    NSLog(@"**** wavSavePath=%@", wavSavePath);
    
    if ([VoiceConverter ConvertAmrToWav:amrPath wavSavePath:wavSavePath]){
        NSLog(@"安卓的amr转wav成功");
    }else
    {
        NSLog(@"安卓的amr转wav失败");
    ...
    [[AVAudioSession sharedInstance] setCategory: AVAudioSessionCategoryPlayback error:nil];
    [[AVAudioSession sharedInstance] setActive:YES error:nil];
    self.player = [self.player initWithContentsOfURL:[NSURL URLWithString:wavSavePath] error:nil];
    NSLog(@"安卓的amr转wav, play");
    [self.player play];
}
@end
